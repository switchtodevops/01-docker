...
Run three containers side by side — one for each layer of a typical 3-tier app — and observe isolation:

```bash
# Web tier
docker run -d --name web -p 8080:80 nginx:latest

# App tier (example Node.js app)
docker run -d --name app -p 3000:3000 node:18-alpine node -e "require('http').createServer((q,r)=>r.end('Hello')).listen(3000)"

# Database tier
docker run -d --name db -e MYSQL_ROOT_PASSWORD=secret mysql:8
```

Verify all three run independently:

```bash
docker ps
```

**Success criteria:** Three containers running with different images, ports, and dependencies — no conflicts on the same host.

---

## 5. Common Mistakes

### Mistake 1: Treating Docker as a VM Replacement for Everything

**Problem:** Running a full OS with systemd, multiple services, and SSH inside one container.

**Why it happens:** Teams familiar with VMs replicate VM patterns inside containers.

```bash
# ❌ Wrong — treating container like a VM
docker run -it ubuntu:latest /bin/bash
apt-get update && apt-get install -y nginx mysql-server openssh-server
# Multiple services, no isolation between them
```
...
> 💡 Containers should run **one main process**. Use Docker Compose or Kubernetes to coordinate multiple containers.

---

### Mistake 2: Ignoring Environment Inconsistency — Hardcoding Config

**Problem:** Baking environment-specific values (DB host, API keys) into the image.

**Why it happens:** Easier to hardcode during development; breaks portability.

```dockerfile
# ❌ Wrong — prod DB host baked into image
ENV DATABASE_URL=prod-db.company.internal:5432
```

```dockerfile
# ✅ Correct — config injected at runtime
ENV DATABASE_URL=""
# Pass at run time: docker run -e DATABASE_URL=qa-db:5432 myapp
```
...
**Problem:** Default container user is root — security risk if the app is compromised.

```dockerfile
# ❌ Wrong — runs as root (default)
FROM node:18-alpine
COPY . /app
CMD ["node", "server.js"]
```
...
### Mistake 4: Assuming Containers Are Fully Secure by Default

**Problem:** Believing container isolation equals VM-level security without additional hardening.

**Why it happens:** Marketing around "secure isolation" is misunderstood.

**How to fix:**
- Run containers as non-root
- Use read-only filesystems where possible
- Scan images for CVEs in CI
- Run containers inside VMs or use rootless Docker/Podman for stronger isolation
- Apply network policies (especially in Kubernetes)
...
| Practice | Why |
|----------|-----|
| Use official or trusted base images | Reduces supply-chain risk |
| Scan images in CI (Trivy, Snyk, Docker Scout) | Catch CVEs before deployment |
| Run as non-root `USER` | Limits blast radius of exploits |
| Pin image tags or digests | Avoid surprise breaking changes |
| Do not store secrets in images or Dockerfiles | Use secrets managers or runtime env injection |
| Keep host kernel patched | Containers share the kernel — host CVEs affect all containers |

### Performance Optimization

- Use **multi-stage builds** to keep production images small
- Prefer **Alpine** or **distroless** base images where appropriate
- Leverage **build cache** — order Dockerfile instructions from least to most frequently changed
- Use `.dockerignore` to exclude unnecessary files from build context

### Maintainability

- One `Dockerfile` per service — version-controlled alongside application code
- Use **labels** for metadata (version, maintainer, build date)
- Document how to build and run in a `README.md` or `docker-compose.yml`

### Production Recommendations

```
Developer Laptop  →  CI Pipeline  →  Container Registry  →  Runtime (VM/K8s)
     docker build       scan + test        push tagged image       deploy immutable image
```

- **Never patch running containers** — rebuild and redeploy
- Use private registries (Azure Container Registry, AWS ECR) in production
- Implement health checks and resource limits
- Plan for stateful data with volumes — containers are ephemeral by design
...
### Exercise: Map Your Current Environment to Docker

**Task:** Document one application from your current or past workplace and analyze how Docker would change the deployment model.

**Requirements:**

1. Draw (ASCII or diagram tool) the **current** deployment stack (hardware → OS → dependencies → app).
2. List at least **3 problems** from the traditional infrastructure table that apply to that app.
3. Redraw the stack using **Docker containers** (one container per tier if multi-tier).
4. Write a short paragraph explaining which of the **6 Docker advantages** (Flexible, Lightweight, Portable, Loosely Coupled, Scalable, Secure) apply and why.
5. Identify one **security consideration** you would address before running that app in production containers.

**Success criteria:**
- Clear before/after architecture comparison
- Problems mapped to Docker solutions
- At least 3 advantages explained with real context
- One security practice identified

> 📌 Do not submit a solution — this is for self-reflection. Discuss your analysis with a peer or revisit after completing Module 03 (Build and Push).
